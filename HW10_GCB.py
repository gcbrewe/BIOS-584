...
#Logistic Regression (set max_iter=1000)
...
#Logistic
logistic_probs_trn = logistic_clf.predict_proba(eeg_trn_signal)
try:
    idx_target_log = list(logistic_clf.classes_).index(1)
except ValueError:
    idx_target_log = int(np.where(np.array(logistic_clf.classes_) == 1)[0][0])
logistic_y_trn = logistic_probs_trn[:, idx_target_log][:, np.newaxis]

#LDA
lda_probs_trn = lda_clf.predict_proba(eeg_trn_signal)
idx_target_lda = list(lda_clf.classes_).index(1)
lda_y_trn = lda_probs_trn[:, idx_target_lda][:, np.newaxis]

#SVM
svm_probs_trn = svm_clf.predict_proba(eeg_trn_signal)
idx_target_svm = list(svm_clf.classes_).index(1)
svm_y_trn = svm_probs_trn[:, idx_target_svm][:, np.newaxis]
...
#Logistic
logistic_probs_frt = logistic_clf.predict_proba(eeg_frt_signal)
logistic_y_frt = logistic_probs_frt[:, idx_target_log][:, np.newaxis]

#LDA
lda_probs_frt = lda_clf.predict_proba(eeg_frt_signal)
lda_y_frt = lda_probs_frt[:, idx_target_lda][:, np.newaxis]

#SVM
svm_probs_frt = svm_clf.predict_proba(eeg_frt_signal)
svm_y_frt = svm_probs_frt[:, idx_target_svm][:, np.newaxis]

print('Shapes:')
print('logistic_y_trn', logistic_y_trn.shape, 'lda_y_trn', lda_y_trn.shape, 'svm_y_trn', svm_y_trn.shape)
print('logistic_y_frt', logistic_y_frt.shape, 'lda_y_frt', lda_y_frt.shape, 'svm_y_frt', svm_y_frt.shape)
...
print('TRN results:')
print('logistic:', logistic_trn_accuracy)
print('lda:', lda_trn_accuracy)
print('svm:', svm_trn_accuracy)

print('FRT results:')
print('logistic:',logistic_frt_accuracy)
print('lda:',lda_frt_accuracy)
print('svm:', svm_frt_accuracy)
...
#Rows 122, 131, 141, 150, 160, and 169 determine for each classifier, in the TRN and FRT sets, the proportion of correct characters in each repetition.

# Step 5: Summary
# Which method performs the best? Why?

#SVM performs the best overall. In TRN all of the methods get to 100% accuracy, but SVM has the highest first-repetition accuracy. In FRT SVM is the best because it is the only one to get to 100% accuracy.